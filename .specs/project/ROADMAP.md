...
**Sintoma concreto confirmado em uso (2026-06-25, logo após o M12 tornar as alturas dinâmicas):** arrastando o card C para entre A e B, a barra de inserção **pula** entre o gap A–B e os gaps adjacentes conforme a posição do hover — em alguns pontos ela encosta na **borda superior de B**, subindo um pouco fica no meio (correto), subindo mais encosta na **borda inferior de A**. Raiz provável: `nearestSlot` (`lib/slots.ts:104`) escolhe o slot por **distância euclidiana ao centro** do slot; com alturas variáveis as regiões "qual gap vence" ficam assimétricas (cards curtos têm gaps acima/abaixo quase equidistantes → flip com micro-movimentos). **Direção de fix a avaliar no design:** seleção por **banda vertical** (em qual faixa Y o cursor cai → qual gap) em vez de distância ao centro; e/ou `PLACEHOLDER_H` deixar de ser fixo. Não é regressão do M12 (escopo explicitamente deferido), mas o M12 o tornou mais visível. Verificado também que **não há sobreposição real de cards** — é só a barra-fantasma. **Não depende de M15**, mas o M15/resize revisita a mesma `lib/slots.ts`/ghost no eixo da **largura** (ortogonal ao fix vertical do M13) — quem vier depois assenta sem conflito. (Se incomodar antes, cabe um fix barato standalone, ciente de que será revisitado.)
