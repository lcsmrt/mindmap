...
**Current Work:** M7 — migração do canvas de React Flow → visx (AD-015) — **executado (T1–T6 ✅), pendente Review AD-013**. Stack do canvas agora: `@visx/zoom` (pan/zoom via @use-gesture), `@visx/shape` (edges `LinkVertical` SVG), `d3-flextree` (layout síncrono de altura variável); `@xyflow/react` e `elkjs` (+ `layout.worker.ts`/`useLayoutedTree.ts`/`treeLayout.ts`) **removidos**. Arquitetura híbrida SVG-edges / HTML-nodes, fonte única `positioned` (Fragile Area `rfNodes`↔`useNodesState` eliminada — M7-24), input inline controlado (M7-25), drag-to-reparent por pointer events. Gates verdes: typecheck + lint + unit (web 41, api 42) + e2e (22/22); smoke manual no browser (render/edges/inline-edit/fitView/pan/zoom/drag) OK. Antes disso: M6 **concluído e aprovado** (review AD-013, 2026-06-23 — ver L-003); US-01 a US-05 de v1 entregues via M1–M6. **Gate de qualidade v1 (RNF-01 500 nós + smoke manual das US) segue adiado conscientemente.** Escopo M7 restrito a paridade funcional + remoção da logo; queixas de UX do canvas ficam como decisão pós-conversão (ver Deferred Ideas). Próximo passo: abrir chat de Review AD-013 do M7.
