...
- **(M12, spec própria) Card responsivo + resize:** hoje `NODE_WIDTH=180` fixo e altura por fórmula (`nodeSize.ts:4-10`); título com `truncate` (`MindNode.tsx:190`); layout puro `computeTreeLayout` recebe só a altura (`useTreeLayout.ts:72`). Exige pipeline **medir→layout** (d3-flextree já suporta tamanho variável por nó) + largura por nó (`NODE_WIDTH` em useTreeLayout/slots/ghost-bar/render) + persistência da largura (campo novo? revisa AD-002 parcialmente) + handle de resize. Decisão já fixada em conversa: **largura controlada, altura derivada**.
...
- **(M14, muito design) Hierarquia visual:** skin por profundidade (`cardSkin(depth)`, `MindNode.tsx:44`); edge herdar cor do nó de 1º nível (hoje `stroke-border` fixo em `MapCanvas.tsx:115`, `LayoutLink` sem cor — `useTreeLayout.ts:15`); "central maior" fica trivial após M12.
